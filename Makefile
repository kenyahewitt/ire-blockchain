...
install:
	...
	@go install $(BUILD_FLAGS) -mod=readonly ./cmd/ire-blockchaind

build:
	@mkdir -p build
	@go build $(BUILD_FLAGS) -mod=readonly -o build/ired ./cmd/ire-blockchaind
...
.PHONY: all install build start status
